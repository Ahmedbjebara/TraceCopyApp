...
import org.apache.hadoop.io.compress.{BZip2Codec, CompressionCodec, CompressionCodecFactory, GzipCodec}
...
/** A facility to deal with file manipulations (wrapper around hdfs apache
  ...
  * @since 2017-02
  */
object HdfsHelper2 extends Serializable {
  ...
  implicit class SeqExtensions[T <: Seq[String]: ClassTag](seq: T) {
    ...
    def writeToHdfs(filePath: String): Unit =
      HdfsHelper2.writeToHdfsFile(seq, filePath)
  }
  ...
  implicit class StringExtensions(string: String) {
    ...
    /** Saves the String in a file on hdfs.
      ...
      */
    def writeToHdfs(filePath: String): Unit =
      HdfsHelper2.writeToHdfsFile(string, filePath)
  }
